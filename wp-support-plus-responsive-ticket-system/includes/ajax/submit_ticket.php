    ...
    foreach ( $form_fields as $field ){
        ...
        if( in_array(0, $categories) || in_array($category, $categories) ){
            ...
            if( isset($_POST['cust_'.$field->id]) && !is_array($_POST['cust_'.$field->id]) ){
                ...
                //Pare Add update woocomerce status to be refund request or open claim status or open legal case status
				//Pare it needs to send SMS and email to both customer and vendor once they have opened the ticket.
                $values['cust'.$field->id] = $save_value;

            }
        }
    }
    //Pare Add function here!!!!!
